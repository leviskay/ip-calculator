			...
			<div class="space-y-4">
				...
				<div class="flex space-x-4">
					<button
						...
						:disabled="!isValid"
						:class="[
							'flex-1 px-4 py-2 rounded-md transition-colors duration-200',
				...
				<div v-if="!networkInfo" class="mt-8">
					...
					<div class="bg-blue-50 p-4 rounded-md text-sm">
						...
						<p class="mt-2 text-xs"><span class="font-bold text-indigo-600">Полужирным</span> выделена сетевая часть адреса</p>
					</div>
				</div>
			</div>
		</div>
...
<script setup>
	...
	function formatBinaryIP(ip, cidr, isNetworkPart = true) {
		...
		for (let i = 0; i < binaryString.length; i++) {
			...
			if ((currentBit < networkBits && isNetworkPart) || (currentBit >= networkBits && !isNetworkPart)) {
	...
	function calculateNetwork() {
		try {
			...
			if (subnetMask.value.startsWith('/')) {
				...
				const parts = subnetMask.value.split('.')
				let binaryMask = parts.map((part) => parseInt(part).toString(2).padStart(8, '0')).join('')
